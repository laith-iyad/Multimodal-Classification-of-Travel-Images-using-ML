...
print("Model: Logistic Regression (Improved)")
for target in targets:
    ...
    print(f"\n{target} | Best {grid.best_params_} | Acc={accuracy_score(yte,pred):.4f} | Macro-F1={f1_score(yte,pred,average='macro'):.4f}")
    ...
    ax.set_title(f"LogReg - {target}")
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / f"cm_logreg_{target.replace(' ', '_').lower()}.png", dpi=250)
    plt.close()

    # Visualize Misclassified Images (Activity & Time of Day)
    if target in ["Activity", "Time of Day"]:
        ...
        if len(mis_indices) > 0:
            ...
            plt.tight_layout()
            plt.savefig(PLOTS_DIR / f"misclassified_logreg_{target.replace(' ', '_').lower()}.png", dpi=200)
            plt.close()
